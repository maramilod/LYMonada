...
const AdminInterface = () => {
 ...
 useEffect(() => {
    ...
    const unsubscribe = firebaseInstance.firestore().collection('users')
      .onSnapshot(snapshot => {
 ...
 const sendReply = async (e) => {
    ...
    try {
      await firebaseInstance.firestore().collection('users').doc(selectedMessage.id).update({
 ...
 const deleteMessage = async (messageId) => {
    try {
      await firebaseInstance.firestore().collection('users').doc(messageId).delete();
 ...
 const deleteEmailAndMessages = async (email) => {
    try {
      ...
      for (const message of messagesToDelete) {
        await firebaseInstance.firestore().collection('users').doc(message.id).delete();
      }
 ...
 return (
    <div className="admin-interface">
      ...
      {Object.entries(groupedMessages).map(([email, messages]) => (
        <div key={email}>
          <h3 className="email-button" onClick={() => handleEmailClick(email)}>
            {email}
          </h3>

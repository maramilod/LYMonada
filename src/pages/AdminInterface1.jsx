...
const AdminInterface = () => {
 ...
 useEffect(() => {
    const fetchMessages = async () => {
      try {
        const usersSnapshot = await firebaseInstance.firestore().collection('users1').get();
    ...
    const unsubscribe = firebaseInstance.firestore().collection('users1')
      .onSnapshot(snapshot => {
 ...
 const sendReply = async (e) => {
    ...
    try {
      await firebaseInstance.firestore().collection('users1').doc(selectedMessage.id).update({
 ...
 const deleteMessage = async (messageId) => {
    try {
      await firebaseInstance.firestore().collection('users1').doc(messageId).delete();
 ...
 const deleteEmailAndMessages = async (email) => {
    try {
      ...
      for (const message of messagesToDelete) {
        await firebaseInstance.firestore().collection('users1').doc(message.id).delete();
      }
 ...
 return (
    <div className="admin-interface1">
      <h2>Messages</h2>
      <input
        type="text"
        value={searchQuery}
      ...
      {Object.entries(groupedMessages).map(([email, messages]) => (
        <div key={email}>
          <h3 className="email-button-1" onClick={() => handleEmailClick(email)}>
            {email}
          </h3>
      ...
      {selectedMessage && (
        <div>
          {selectedMessage ? <p>Selected Message: {selectedMessage.text}</p> : null}
